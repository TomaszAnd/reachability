...
class SweepConfig:
    ...
    restarts: int = 3
    method: str = 'L-BFGS-B'
    krylov_m: Optional[int] = None  # None = use dim
...
def _make_submodel(model: QuantumModel, K: int, seed: int) -> QuantumModel:
    """Create a submodel with a specific seed, preserving model type behavior."""
    # Create a fresh model with the given seed to get reproducible submodels
    from .models import CanonicalModel, GeometricTwoLocalModel, _SubModel

    if isinstance(model, CanonicalModel):
        fresh = CanonicalModel(
            model.dim, include_identity=model.include_identity, seed=seed)
        return fresh.sample_submodel(K)
    elif isinstance(model, GeometricTwoLocalModel):
        fresh = GeometricTwoLocalModel(
            model.dim, nx=model.nx, ny=model.ny,
            periodic=model.periodic, seed=seed)
        return fresh.sample_submodel(K)
    else:
        # Generic fallback: use model's own sample_submodel
        return model.sample_submodel(K)

"""
Optimization utilities for maximizing spectral overlap.
...
def maximize_spectral_overlap(
    ...
    objective = create_objective_function(psi, phi, hams, bounds, method)
    ...
    logger.debug(f"Starting optimization: method={method}, restarts={restarts}, maxiter={maxiter}")
    ...
    for restart_idx in range(restarts):
        ...
        try:
            # Configure optimization based on method capabilities
            options = {"maxiter": maxiter, "ftol": ftol}

            if registry[method]["supports_bounds"]:
                # Method supports bounds natively
                result = minimize(objective, x0, method=method, bounds=bounds, options=options)
            else:
                # Method doesn't support bounds - rely on clipping in objective
            ...
            current_value = -result.fun  # Convert back to positive spectral overlap
            ...
            logger.debug(
                f"Restart {restart_idx + 1}: S={current_value:.6f}, success={result.success}"
            )
    ...
    # Recompute final value with clipped parameters
    try:
        final_value = mathematics.spectral_overlap(best_x, psi, phi, hams)
        best_value = max(best_value, final_value)  # Take the better value
    except Exception as e:
    ...
    logger.debug(
        f"Optimization complete: S*={best_value:.6f}, nfev={total_nfev}, time={runtime_s:.3f}s"
    )

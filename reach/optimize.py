...
def _maximize_criterion(
    ...
    **criterion_kwargs,
    ...
    # Use analytical gradient for spectral/krylov overlap with L-BFGS-B
    use_spectral_grad = (
        criterion_func is mathematics.spectral_overlap
        and method == "L-BFGS-B"
        and hasattr(mathematics, 'spectral_overlap_with_grad')
    )
    use_krylov_grad = (
        criterion_func is mathematics.krylov_score
        and method == "L-BFGS-B"
        and hasattr(mathematics, 'krylov_score_with_grad')
    )
    use_analytical_grad = use_spectral_grad or use_krylov_grad

    # Precompute numpy arrays of Hamiltonians (avoids repeated QObj→numpy conversion)
    ...
    def objective(x: np.ndarray) -> float:
        ...
        try:
            value = criterion_func(x, psi, phi, hams, hams_np=hams_np, **criterion_kwargs)
            return -float(value)
    ...
    def objective_and_grad(x: np.ndarray):
        """Objective function with analytical gradient."""
        try:
            if use_spectral_grad:
                value, grad = mathematics.spectral_overlap_with_grad(
                    x, psi, phi, hams, hams_np=hams_np
                )
            else:  # use_krylov_grad
                value, grad = mathematics.krylov_score_with_grad(
                    x, psi, phi, hams, hams_np=hams_np, **criterion_kwargs
                )
            return -float(value), -grad.astype(np.float64)
        except Exception as e:
            logger.debug(f"Objective+grad evaluation failed at λ={x}: {e}")
    ...
    for restart_idx in range(restarts):
        ...
        try:
            ...
            if use_analytical_grad:
                result = minimize(
                    objective_and_grad, x0, method=method,
                    jac=True, bounds=bounds, options=options,
                )
            elif supports_bounds:
    ...
    # Final clipping and recomputation (only if clipping changed best_x)
    best_x_clipped = clip_to_bounds(best_x, bounds)
    if not np.allclose(best_x_clipped, best_x):
        best_x = best_x_clipped
        try:
            final_value = criterion_func(best_x, psi, phi, hams, hams_np=hams_np, **criterion_kwargs)

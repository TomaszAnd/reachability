...
Unlike the moment criterion, the spectral criterion requires numerical optimization, which may converge to local maxima and gis not guaranteed to coincide with the global optimum. However, it offers a more fine-grained test: rather than checking polynomial constraints on expectation values, it directly examines whether the spectral distributions can be made to match. A further advantage is that optimization is only over the Hamiltonian parameters, not over time, since the probabilities $p_i$ and $q_i$ are time-independent.
